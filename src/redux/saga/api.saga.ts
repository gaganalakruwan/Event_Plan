...
export function* getComments({payload: {success, failed}}:TSaga) {
  try {
    const {data} = yield call(Api.getAllComments);
...
export function* apiSaga() {
  yield takeLatest(CommonTypes.GET_IMAGES, getImages);
  yield takeLatest(CommonTypes.GET_USERS, getUsers);
  yield takeLatest(CommonTypes.GET_POSTS, getPosts);
  yield takeLatest(CommonTypes.GET_COMMENTS, getComments);
}
